...
async function loadLawyers() {
  try {
    if (window.lawyerSearchApi) {
      allLawyers = await window.lawyerSearchApi.list();
      
      // Map API fields to frontend expected fields if needed
      allLawyers = allLawyers.map(l => ({
        ...l,
        name: l.first_name + ' ' + l.last_name,
        bar_no: l.bar_registration_no,
        avatar_text: (l.first_name[0] + l.last_name[0]).toUpperCase(),
        categories: (l.categories || []).map(c => String(c.category_id)),
        category_names: (l.categories || []).map(c => c.category_name),
        packages: l.package_summary || [],
        packages_count: l.package_summary ? l.package_summary.length : 0,
        reviews_count: 0 // Mocked for now, not tracked in DB
      }));
    }
  } catch (err) {
    console.error('Failed to load lawyers:', err);
    LC.showToast('Failed to load lawyers.', 'error');
  }
}
...
function parseUrlQueryParams() {
  ...
  if (lawyerIdParam) {
    const target = allLawyers.find(l => String(l.lawyer_id) === String(lawyerIdParam));
    if (target) {
